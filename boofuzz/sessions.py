        ...
        if len(target.monitors) > 0:
            ...
            for monitor in target.monitors:
                if not monitor.post_send(target=target, fuzz_data_logger=self._fuzz_data_logger, session=self):
                    ...
                    if hasattr(monitor, "get_corefile"):
                        data = monitor.get_corefile()
                        remote_process_name = monitor.server_options["proc_name"]
                        if data:
                            corefile = "/home/calmk/savecore/" + "core-" + remote_process_name + "-" +str(self.total_mutant_index) + ".core"
                            with open(corefile, "wb") as f:
                                f.write(data)
                            self._fuzz_data_logger.log_fail(f"save {remote_process_name} corefile to {corefile}, crash by testcase #{self.total_mutant_index}")
                        else:
                            self._fuzz_data_logger.log_fail(f"do not have corefile for {remote_process_name}, crash by testcase #{self.total_mutant_index}")
