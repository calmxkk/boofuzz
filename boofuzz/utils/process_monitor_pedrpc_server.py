...
class ProcessMonitorPedrpcServer(pedrpc.Server):
    ...
    def get_crash_synopsis(self):
        ...
        self.log(f"get crash synopsis: {self.last_synopsis}", 10)

        return self.last_synopsis

    def get_corefile(self):
        """
        Return the last corefile.
        Transfer the coredump file from the server to the client

        @rtype:  String
        @return: Synopsis of last recorded crash.
        """
        if not self.last_corefile:
            self.log("do not have the last corefile", 10)
            return None
        self.log(f"move corefile to client, file:{self.last_corefile}", 10)
        with open(self.last_corefile, "rb") as f:
            return f.read()
    ...
    def pre_send(self, test_number):
        ...
        if self.debugger_thread is None or not self.debugger_thread.is_alive():
            self.log(f"target is not alive, stop create new debugger thread, wait restart", 10)
            # self.start_target()
