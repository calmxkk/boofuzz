...
class DebuggerThreadSimple(threading.Thread):
        ...
        for command in self.start_commands:
            ...
            try:
                if self.capture_output:
                    self._process = subprocess.Popen(command, stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
                else:
    ...
    def run(self):
        ...
        if self.proc_name:
            ...
            self.log(f"process {self.proc_name}-{self.pid} terminate!")
        ...
        msg = "[{0}] Crash. Exit code: {1}. Reason - {2}\n".format(
            time.strftime("%Y-%m-%d %H:%M:%S"), self.exit_status if self.exit_status is not None else "<unknown>", reason
        )
    ...
    def get_corefile(self):
        with open('/proc/sys/kernel/core_pattern', 'r') as f:
            core_pattern = f.read().strip()
        
        core_dir = os.path.dirname(core_pattern)
        core_pattern = core_pattern.replace('%e', self.proc_name) \
                        .replace('%p', str(self.pid)) \
                        .replace('%u', '[0-9]*') \
                        .replace('%g', '[0-9]*') \
                        .replace('%s', '[0-9]*') \
                        .replace('%t', '[0-9]*') \
        ...
        corefile_regex = re.compile(core_pattern)
        
        for file in os.listdir(core_dir):
            file_path = os.path.join(core_dir, file)

            if corefile_regex.match(file_path):
                return file_path
        
        return None

    def save_corefile(self):
        if self.process_monitor.coredump_dir is not None:
            corefile = self.get_corefile()
            if corefile:
                filename = os.path.basename(corefile) + "-" + str(self.process_monitor.test_number)
                dest = os.path.join(self.process_monitor.coredump_dir, filename)

                self.log("moving core dump %s -> %s" % (corefile, dest))
                shutil.copyfile(corefile, dest)
                self.process_monitor.last_corefile = os.path.abspath(dest)
                self.log(f"save last corefile path for transform {self.process_monitor.last_corefile}", 10)
            else:
                self.log("can not find corefile")
                self.process_monitor.last_corefile = ""
    ...
    def post_send(self):
        ...
        """
        if self.is_alive():
            self.log(f"target is alive, return True.", 20)
            return True
        else:
            self.log(f"target is not is_alive, ...")
